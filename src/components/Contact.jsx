      ...
      ScrollTrigger.matchMedia({
        "(min-width: 769px) and (prefers-reduced-motion: no-preference)":
          () => {
            ...
            const chars =
              containerRef.current.querySelectorAll(".contact-char");
            const words =
              containerRef.current.querySelectorAll(".contact-word");
    ...
    <section id="contact" className="contact-section" ref={containerRef}>
      <div className="contact-pin">
        ...
        <p className="contact-subtitle">
          {splitWords([SUBTITLE_LINE_1, SUBTITLE_LINE_2]).map(
            (lineWords, lineIndex) => (
              <span key={lineIndex}>
                {lineWords}
                {lineIndex === 0 && <br />}
